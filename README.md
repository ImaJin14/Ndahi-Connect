...
For production, edit the single `.env`, set `NODE_ENV=production`, and replace every local or placeholder value. The API validates this configuration at startup and refuses to run with mock adapters, JSON-file persistence, insecure URLs/cookies, placeholder secrets, disabled administrator MFA, or an unconfigured provider. Set `PAYMENT_MODE=mesomb` to expose only MeSomb while retaining the dormant Flutterwave integration. Configure MeSomb's signed webhook as `https://api.ndahiconnect.net/api/webhooks/mesomb`; set `PAYMENT_MODE=flutterwave` later to switch back without removing either adapter. Resend delivers the verified-payment voucher confirmation; set `EMAIL_MODE=live`, `EMAIL_API_URL=https://api.resend.com`, `EMAIL_API_KEY`, and a verified `EMAIL_FROM` address.

For production persistence, create a PostgreSQL database, set `DATABASE_URL`, then run `npm run migrate:postgres`. Migrations run in filename order with `ON_ERROR_STOP`, and each schema migration is transactional. The API stores customers, vouchers, payments, sessions, bundles, challenges, events, and audit logs in normalized PostgreSQL tables while retaining atomic JSON-file persistence for local development. Set `DATABASE_SSL=false` only for a trusted local instance or Render's same-region private database URL; external database connections must use TLS. Existing `ndahi_state` data is validated, migrated, and count-reconciled transactionally; follow the zero-loss [migration and rollback runbook](docs/operations/normalized-state-migration-runbook.md) before starting the normalized API version.
