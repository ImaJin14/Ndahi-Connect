import { receiptDocument } from "./billing.mjs";
import { createHash } from "node:crypto";

const hashForIdempotency = (value) =>
...
export class ResendEmailAdapter {
  ...
  async sendReceipt({ to, receipt }) {
    if (!this.configured() || !to) throw Error("Receipt email delivery is not configured.");
    const html = receiptDocument(receipt);
    const response = await this.fetcher(`${this.apiUrl}/emails`, {
      method: "POST", signal: AbortSignal.timeout(10000),
      headers: { authorization: `Bearer ${this.apiKey}`, "content-type": "application/json",
        "idempotency-key": `payment-receipt/${receipt.paymentId}` },
      body: JSON.stringify({ from: this.from, to: [to], subject: `NDAHI Connect receipt ${receipt.number}`,
        html, text: `Receipt ${receipt.number}\nPackage: ${receipt.plan.name}\nPaid: ${receipt.amount} ${receipt.currency}\nProvider: ${receipt.provider}\nReference: ${receipt.providerReference || "Not recorded"}\nDownload your receipt from ${this.portalUrl}/dashboard.` }),
    });
    if (!response.ok) throw Error("Receipt email delivery failed.");
    const result = await response.json();
    return { messageId: result.id };
  }

  async sendSecurityAlert({ to, alert }) {
    if (!this.configured() || !to) throw Error("Security alert email delivery is not configured.");
    const subject = alert.subject,
      text = [`Hello,`, "", ...alert.lines, "", "If this wasn't you, sign in and review your account security, or contact support."].join("\n"),
      html = `<!doctype html><html><body style="font-family:Arial,sans-serif;color:#082d25;line-height:1.5"><div style="max-width:560px;margin:auto;padding:28px"><h1 style="font-size:22px">${escapeHtml(subject)}</h1>${alert.lines.map((line) => `<p>${escapeHtml(line)}</p>`).join("")}<p style="font-size:13px;color:#587069">If this wasn't you, sign in and review your account security, or contact support.</p></div></body></html>`;
    const response = await this.fetcher(`${this.apiUrl}/emails`, {
      method: "POST", signal: AbortSignal.timeout(10000),
      headers: { authorization: `Bearer ${this.apiKey}`, "content-type": "application/json",
        "idempotency-key": `security-alert/${alert.id}` },
      body: JSON.stringify({ from: this.from, to: [to], subject, html, text }),
    });
    if (!response.ok) throw Error("Security alert email delivery failed.");
    const result = await response.json();
    return { messageId: result.id };
  }

  async sendPinReset({ customer, token }) {
    if (!this.configured()) throw new Error("Email provider is not configured.");
    if (!customer?.email) throw new Error("Customer email is missing.");
    const resetUrl = `${this.portalUrl}/forgot-pin?token=${encodeURIComponent(token)}`,
      subject = "Reset your NDAHI Connect PIN",
      text = `Hello ${customer.name || "Customer"},\n\nUse this secure link within 15 minutes to reset your NDAHI Connect PIN:\n${resetUrl}\n\nIf you did not request this, you can ignore this email.`,
      html = `<!doctype html><html><body style="font-family:Arial,sans-serif;color:#082d25;line-height:1.5"><div style="max-width:560px;margin:auto;padding:28px"><p>Hello ${escapeHtml(customer.name || "Customer")},</p><h1 style="font-size:24px">Reset your PIN</h1><p>This secure link expires in 15 minutes and can only be used once.</p><p><a href="${escapeHtml(resetUrl)}" style="display:inline-block;background:#b4f228;color:#082d25;padding:12px 18px;border-radius:8px;text-decoration:none;font-weight:700">Reset PIN</a></p><p>If you did not request this, you can ignore this email.</p></div></body></html>`;
    const response = await this.fetcher(`${this.apiUrl}/emails`, {
      method: "POST",
      signal: AbortSignal.timeout(10000),
      headers: {
        authorization: `Bearer ${this.apiKey}`,
        "content-type": "application/json",
        "idempotency-key": `pin-reset/${hashForIdempotency(token)}`,
      },
      body: JSON.stringify({ from: this.from, to: [customer.email], subject, html, text }),
    });
    let result = {};
    try { result = await response.json(); } catch { /* provider returned no JSON */ }
    if (!response.ok) throw new Error(`Email delivery failed: ${String(result.message || response.statusText || response.status).slice(0, 200)}`);
...
export class MockEmailAdapter {
  async sendReceipt({ receipt }) { return { messageId: `receipt-${receipt.paymentId}` }; }
  configured() { return true; }
  async sendVoucher({ voucher }) { return { messageId: `mock-${voucher.id}` }; }
  async sendPinReset() { return { messageId: "mock-pin-reset" }; }
  async sendSecurityAlert({ alert }) { return { messageId: `mock-alert-${alert.id}` }; }
}
