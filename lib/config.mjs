...
export function productionConfigErrors(env = process.env) {
  ...
  if (env.OTP_DELIVERY !== "live") errors.push("OTP_DELIVERY must be live");
  requireHttps(env, "SMS_API_URL", errors);
  ...
  if (env.PAYMENT_MODE !== "live") errors.push("PAYMENT_MODE must be live");
...
export function enabledPaymentProviders(env = process.env) {
  ...
  return env.PAYMENT_MODE === "live" ? ["flutterwave"] : [];
}
