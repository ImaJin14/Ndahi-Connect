...
export function productionConfigErrors(env = process.env) {
  ...
  for (const [current, previous] of [
    ...
    const value = String(env[previous] || "").trim();
    if (value && value.length < 32) errors.push(`${previous} must be at least 32 characters`);
    if (value && value === String(env[current] || "").trim()) {
      errors.push(`${previous} must differ from ${current}`);
    }
  }
  ...
  if (env.NETWORK_ROUTER_SOURCE !== "saved") {
    requireHttps(env, "MIKROTIK_API_URL", errors);
    requireValue(env, "MIKROTIK_USER", errors);
    requireValue(env, "MIKROTIK_PASSWORD", errors);
  }
  if (env.NETWORK_ROUTER_SOURCE === "saved" || env.NETWORK_OMADA_SOURCE === "saved" || env.NETWORK_PROVISIONING_ENABLED === "true") {
    if (!/^[a-fA-F0-9]{64}$/.test(env.NETWORK_CONFIG_KEY || "")) errors.push("NETWORK_CONFIG_KEY must be a 32-byte hexadecimal key");
    const managementOrigins = String(env.NETWORK_ALLOWED_ORIGINS || "").split(",").map((v) => v.trim()).filter(Boolean);
    if (!managementOrigins.length || managementOrigins.some((v) => { try { const u = new URL(v); return u.protocol !== "https:" || u.origin !== v; } catch { return true; } })) errors.push("NETWORK_ALLOWED_ORIGINS must list explicit HTTPS origins");
  }
  if (env.OMADA_MODE !== "live") errors.push("OMADA_MODE must be live");
  if (env.NETWORK_OMADA_SOURCE !== "saved") {
    requireHttps(env, "OMADA_API_URL", errors);
    requireValue(env, "OMADA_API_TOKEN", errors);
  }
  if (!["mesomb", "flutterwave", "live"].includes(env.PAYMENT_MODE)) {
    errors.push("PAYMENT_MODE must be mesomb or flutterwave");
  }
  if (env.PAYMENT_MODE === "mesomb") {
    requireValue(env, "MESOMB_APPLICATION_KEY", errors);
    requireValue(env, "MESOMB_ACCESS_KEY", errors);
    requireValue(env, "MESOMB_SECRET_KEY", errors);
    requireValue(env, "MESOMB_WEBHOOK_SECRET", errors);
  } else {
...
export function enabledPaymentProviders(env = process.env) {
  ...
  if (env.PAYMENT_MODE === "mesomb") return ["mesomb"];
