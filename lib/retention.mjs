export const retentionPolicy = Object.freeze({
  ...
  securityAlertsDays: 90,
  auditLogsDays: 730,
  providerEventsDays: 730,
  routerCommandsDays: 90,
  networkSetupJobsDays: 90,
...
export async function runRetention(client, now = new Date()) {
  ...
  try {
    ...
    for (const [name, kind, days, archiveDays] of [
      ...
      ["securityAlerts", "security_alert", p.securityAlertsDays, p.operationalArchiveDays],
      ["providerEvents", "provider", p.providerEventsDays, p.auditAndPaymentArchiveDays],
      ["routerCommands", "router_command", p.routerCommandsDays, p.operationalArchiveDays],
      ["networkSetupJobs", "network_setup_job", p.networkSetupJobsDays, p.operationalArchiveDays],
      ...
      where: "kind = $3 AND occurred_at IS NOT NULL AND occurred_at::timestamptz < $4::timestamptz" +
        (kind === "provider" ? " AND (payload->>'kind' IS DISTINCT FROM 'payment_webhook' OR payload->>'status' = 'processed')" : "") +
        (kind === "router_command" ? " AND payload->>'status' = 'processed'" : "") +
        (kind === "network_setup_job" ? " AND payload->>'status' IN ('completed', 'rolled_back', 'recovered_manually', 'failed', 'cancelled', 'superseded', 'preview')" : "") +
        (kind === "security_alert" ? " AND payload->>'status' <> 'pending'" : ""),
