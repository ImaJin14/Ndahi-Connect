...
BILL-001–BILL-004 now add durable checkout reservations, recovery across tabs and
restarts, paid receipts, provider-backed refund tracking, and customer billing rules.
The server enforces one unresolved checkout per customer and checks suspended
accounts before purchase. The original 13 browser scenarios passed again on
2026-09-25. See [billing verification and operating limits](billing-experience.md)
for the implementation, additional checks, and live-provider verification still required.
