...
async function initialize() {
  ...
  for (const payment of account?.payments || []) {
    ...
    if (["paid", "failed", "refunded"].includes(payment.status) && payment.requestKey &&
      sessionStorage.getItem(key) === payment.requestKey) sessionStorage.removeItem(key);
  }
  renderPlans();
  ...
  const pending = accountAction && account.payments?.find((payment) => payment.status === "pending");
  if (pending) {
    ...
    $("#checkExisting").onclick = async () => {
      ...
      try {
        ...
        if (["failed", "refunded"].includes(result.payment.status)) {
          sessionStorage.removeItem(`ndahi-payment-${pending.action || "purchase"}-${pending.planId}`);
          location.reload();
        } else $("#planNotice p").textContent = "This payment is still awaiting approval. Approve the existing Mobile Money prompt, then check its status again.";
      } catch (error) {
...
function openCheckout(trigger) {
  ...
  $("#checkoutPolicy").textContent = policy(selected);
...
function clearRequestKey() {
  if (activeRequestKey) sessionStorage.removeItem(activeRequestKey);
}
...
$("#purchase").onsubmit = async (event) => {
  ...
  try {
    ...
    let requestKey = sessionStorage.getItem(activeRequestKey);
    if (!requestKey) { requestKey = crypto.randomUUID(); sessionStorage.setItem(activeRequestKey, requestKey); }
    const created = await call(action ? "/api/account/plan/purchase" : "/api/purchase", {
      method: "POST",
      body: JSON.stringify({ ...(action ? { network: purchaseInput.network } : purchaseInput), planId: selected.id, upgrade: upgradePurchase, action, requestKey }),
    });
    ...
    $("#message").innerHTML = `<div class="success">Payment request created.${created.checkout.mode === "mock"
