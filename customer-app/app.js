...
const loginLabels = {
  ...
  "customer.otp_login.succeeded": "Authenticator sign-in",
  "customer.passkey_login.succeeded": "Passkey sign-in",
  "customer.recovery_code_login.succeeded": "Recovery code sign-in",
  "customer.access.succeeded": "Voucher activation",
};

async function load() {
  const [result, security] = await Promise.all([
    call("/api/account/dashboard"), call("/api/account/security"),
  ]);
  const active = result.activeBundle,
...
$("#dashboard").onclick = async (event) => {
  ...
  const removePasskeyButton = event.target.closest("[data-remove-passkey]");
  if (removePasskeyButton) {
    if (!confirm("Remove this passkey? It will no longer be able to sign in to your account.")) return;
    try {
      await runButton(removePasskeyButton, "Removing…", async () => {
        await call("/api/account/passkeys/remove", { method: "POST", body: JSON.stringify({ passkeyId: removePasskeyButton.dataset.removePasskey }) });
        await load();
        $("#dashboardMessage").textContent = "Passkey removed.";
      });
    } catch (error) { showError($("#dashboardMessage"), error); }
    return;
  }
  const recoveryButton = event.target.closest("#generateRecoveryCodes");
  if (recoveryButton) {
    try {
      await runButton(recoveryButton, "Generating…", async () => {
        const result = await call("/api/account/security/recovery-codes/generate", { method: "POST", body: "{}" });
        $("#recoveryCodesOutput").innerHTML = `<div class="success"><p>Save these codes now — each works once, and they won't be shown again.</p><ul>${result.codes.map((code) => `<li><code>${h(code)}</code></li>`).join("")}</ul></div>`;
      });
    } catch (error) { showError($("#dashboardMessage"), error); }
    return;
  }
  ...
  if (revokeSessionButton) {
    if (!confirm("Sign out this session? If it's the device you're using now, you'll be signed out too.")) return;
    try {
      await runButton(revokeSessionButton, "Signing out…", async () => {
        const result = await call("/api/account/security/sessions/revoke", { method: "POST", body: JSON.stringify({ sessionId: revokeSessionButton.dataset.revokeSession }) });
        if (result.loggedOut) { location.href = "/login"; return; }
        await load();
        $("#dashboardMessage").textContent = "Session signed out.";
      });
    } catch (error) { showError($("#dashboardMessage"), error); }
    return;
  }
  const logoutEverywhereButton = event.target.closest("#logoutEverywhere");
  if (logoutEverywhereButton) {
    if (!confirm("Log out everywhere? This signs out every device, including this one.")) return;
    try {
      await runButton(logoutEverywhereButton, "Signing out…", async () => {
        await call("/api/account/security/logout-everywhere", { method: "POST", body: "{}" });
        location.href = "/login";
      });
    } catch (error) { showError($("#dashboardMessage"), error); }
    return;
...
$("#dashboard").onsubmit = async (event) => {
  const renameForm = event.target.closest("[data-rename-passkey]");
  if (renameForm) {
    event.preventDefault();
    try {
      await runButton(renameForm.querySelector("button"), "Saving…", async () => {
        await call("/api/account/passkeys/rename", { method: "POST", body: JSON.stringify({ passkeyId: renameForm.dataset.renamePasskey, label: new FormData(renameForm).get("label") }) });
        await load();
        $("#dashboardMessage").textContent = "Passkey renamed.";
      });
    } catch (error) { showError($("#dashboardMessage"), error); }
    return;
  }
